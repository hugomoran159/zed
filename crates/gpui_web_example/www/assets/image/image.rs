...
use anyhow::Result;
use gpui::{
    App, AppContext, Application, Bounds, Context, ImageSource, KeyBinding, Menu,
...
fn main() {
    ...
    app.run(move |cx: &mut App| {
        ...
        #[cfg(not(target_arch = "wasm32"))]
        let local_resource: Arc<std::path::Path> =
            manifest_dir.join("examples/image/app-icon.png").into();
        #[cfg(target_arch = "wasm32")]
        let local_resource: SharedUri = "assets/image/app-icon.png".into();

        #[cfg(not(target_arch = "wasm32"))]
        let asset_resource: SharedString = "image/color.svg".into();
        #[cfg(target_arch = "wasm32")]
        let asset_resource: SharedUri = "assets/image/color.svg".into();
